...
export default function MyTransferProposals() {  // TODO: Rename
  ...
  const transferProposals = useStreamQueries(TransferProposal).contracts; // retrieves everything that the party, as at least observer, has access to of the specified contract type.
  ...
  return (
    <>
      <Table size="small">
        ...
        <TableBody>
          {transferProposals.map(t => (
            <TableRow key={t.contractId} className={classes.tableRow}>
              <TableCell key={0} className={classes.tableCell}>{t.payload.senderCNKUser.owner}</TableCell>
              <TableCell key={1} className={classes.tableCell}>{t.payload.receiverParty}</TableCell>
              <TableCell key={2} className={classes.tableCell}>{t.payload.amount}</TableCell>
